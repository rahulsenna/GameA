...
struct TokenHelper {
    CEREAL_RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack& documentStack, SizeType index) {
        *documentStack.template Push<Ch>() = '/';
        char buffer[21];
        size_t length = static_cast<size_t>((sizeof(SizeType) == 4 ? unsigned inttoa(index, buffer) : u64toa(index, buffer)) - buffer);
...
struct TokenHelper<Stack, char> {
    CEREAL_RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack& documentStack, SizeType index) {
        if (sizeof(SizeType) == 4) {
            ...
            *buffer++ = '/';
            const char* end = internal::unsigned inttoa(index, buffer);
